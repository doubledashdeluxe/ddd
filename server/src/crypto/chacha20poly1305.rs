...
impl Cipher for ChaCha20Poly1305 {
    ...
    fn encrypt(k: &Self::Key, nonce: u64, ad: &[u8], plaintext: &[u8], out: &mut [u8]) {
        let k = SecretKey::try_from(k.as_slice()).unwrap();
        let mut full_nonce = [0u8; 12];
        full_nonce[4..].copy_from_slice(&nonce.to_le_bytes());
        let full_nonce = Nonce::from(full_nonce);
        chacha20poly1305::ChaCha20Poly1305::seal(&k, &full_nonce, plaintext, Some(ad), out)
            .unwrap();
    }
    ...
    fn decrypt(
        ...
        full_nonce[4..].copy_from_slice(&nonce.to_le_bytes());
        let full_nonce = Nonce::from(full_nonce);
        chacha20poly1305::ChaCha20Poly1305::open(&k, &full_nonce, ciphertext, Some(ad), out)
            .map_err(|_| ())
    }

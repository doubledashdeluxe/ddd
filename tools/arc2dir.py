...
def unpack_file(decompress, data, path):
    if decompress:
        data = zlib.decompress(data[4:], wbits = -15)
    with open(path, 'wb') as file:
        file.write(data)

def unpack_dir(decompress, data, files, dirs, nodes, names, path):
    os.mkdir(path)
    ...
    for node_index in range(first_node_index, first_node_index + node_count):
        unpack_node(decompress, nodes[node_index * 0x14:], files, dirs, nodes, names, path)

def unpack_node(decompress, data, files, dirs, nodes, names, path):
    ...
    if is_dir:
        ...
        unpack_dir(decompress, dirs[dir_index * 0x10:], files, dirs, nodes, names, path)
    else:
        file_offset = unpack_u32(data, 0x08)
        file_size = unpack_u32(data, 0x0c)
        unpack_file(decompress, files[file_offset:file_offset + file_size], path)

def unpack_tree(decompress, data, files, path):
    ...
    unpack_dir(decompress, dirs, files, dirs, nodes, names, path)

def unpack_archive(decompress, data, path):
    tree_offset = unpack_u32(data, 0x08)
    files_offset = unpack_u32(data, 0x0c)
    files = data[tree_offset + files_offset:]
    unpack_tree(decompress, data[tree_offset:], files, path)
...
with open(args.in_path, 'rb') as arc_file:
    unpack_archive(args.decompress, arc_file.read(), out_path)

...
Array<char, 256> VirtualCard::getPath(CARDStat &stat) {
    Array<char, 256> path;
    if (!strcmp(stat.fileName, "MarioKart Double Dash!!")) {
        snprintf(path.values(), path.count(), "%s/%.2s-%.4s-%s.gci", m_dirPath.values(),
                stat.company, stat.gameName, stat.fileName);
    } else {
        Array<u8, 0x15> giRecord;
        for (u32 i = 0; i < giRecord.count(); i += 3) {
            Array<u8, 4> values;
            for (u32 j = 0; j < values.count(); j++) {
                values[j] = stat.fileName[1 + i / 3 * 4 + j] - '!';
            }
